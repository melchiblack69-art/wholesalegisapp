...
export default function WarehouseUserProfile() {
  ...
  useEffect(() => {
    Promise.all([
      editing ? api.get(`/api/auth/admins/${userId}`) : api.get(`/api/auth/mycompany/${companyId}`),
    ])
      .then(([user]) => {
        if (!editing && user) {
          setCompanyName(user.company_name || user.companyName || user.company?.company_name || "");
        ...
        if (!editing) setForm((current) => ({ ...current, company_id: companyId || "" }));
      })
      .catch((e) => setError(e.message || "Could not load user profile."))
      .finally(() => setLoading(false));
  }, [editing, userId, companyId]);
  ...
  const submit = async (event) => {
    ...
    setError("");
    try {
      if (editing) await api.put(`/api/auth/update/${userId}`, form);
      else await api.post("/api/auth/admins", { ...form, company_id: companyId });
       showModal("Account saved successful.", { type: "success", 
        title: "Success", autoClose: true,
         confirmText: false, autoCloseDelay: 1000});
         setTimeout(()=>{
           navigate(`/company/${form.company_id || companyId}/users`);
         }, 1300);
    } catch (e) {
            ...
            <div className="col-xl-4">
              <div className="card-surface user-profile-summary h-100">
                ...
                <div className="user-profile-summary-line mt-4">
                  ...
                  <span>
                    {companyName || form.company_name || form.companyName || (form.company_id ? `Company ${form.company_id}` : "No company assigned")}
                  </span>
                </div>
              ...
              <div className="card-surface p-4 p-lg-5">
                ...
                <div className="row g-3">
                  ...
                  <div className="col-md-6">
                    <label className="form-label">Access role <i className="bi bi-asterisk text-danger" style={{fontSize: "0.55rem" }}></i></label>
                    <select
                ...
                <div className="profile-form-footer">
                  <button
                    ...
                    onClick={() => navigate(`/company/${form.company_id}/users`)}
                  >
                    Cancel
                  </button>
